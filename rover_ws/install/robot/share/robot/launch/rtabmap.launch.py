...
def launch_setup(context, *args, **kwargs):      
    ...
    return [
        DeclareLaunchArgument('depth', default_value=ConditionalText('false', 'true', IfCondition(PythonExpression(["'", LaunchConfiguration('stereo'), "' == 'false'"]))._predicate_func(context)), description=''),
...
def generate_launch_description():
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument('queue_size',     default_value='100',                 description='Backward compatibility, use "sync_queue_size" instead.'),
        ...
        DeclareLaunchArgument('wait_for_transform', default_value='0.5',            description=''),
        DeclareLaunchArgument('rtabmap_args',   default_value='',                   description='Backward compatibility, use "args" instead.'),
        DeclareLaunchArgument('launch_prefix', default_value='stdbuf -o L', description='For debugging purpose, it fills prefix tag of the nodes, e.g., "xterm -e gdb -ex run --args"'),
        ...
        DeclareLaunchArgument('use_action_for_goal', default_value='true',         description='Connect to nav2\'s navigate_to_pose action server instead of publishing the output goal topic.'),
        ...
        DeclareLaunchArgument('rgb_topic',           default_value='/d430/color/image_raw',       description=''),
        DeclareLaunchArgument('depth_topic',         default_value='/d430/depth/image_rect_raw', description=''),
        DeclareLaunchArgument('camera_info_topic',   default_value='/d430/depth/camera_info',            description=''),
        ...
        DeclareLaunchArgument('depth_scale',      default_value='0.001',        description=''),
        ...
        DeclareLaunchArgument('scan_cloud_topic',     default_value='/d430/depth/points', description=''),
        ...
        #GPS
        DeclareLaunchArgument('gps_topic',  default_value='/gps', description='GPS async subscription. This is used for SLAM graph optimization and loop closure candidates selection.'),
